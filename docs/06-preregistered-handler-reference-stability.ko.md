...
그렇지 않으면 라이브러리 내부의 `interceptionState` 상태가 초기화되어,
...
`interceptionState`가 새로 생성됨.

**주의**: `currentOverlayRef`(ref)를 사용하는 것은 함수 내부에서 최신 값을 참조하기 위한 것이지,
함수 참조 자체를 안정화하지 않습니다.

이로 인해 이전 async IIFE에서 설정한 `next-navigation confirmation` 상태가 유실됨.
...
export function useInterceptPopState({
  ...
  }, [handlerMap, pagesRouter, preRegisteredHandler]);  // ← preRegisteredHandler가 deps에 있음
}

const createPopstateInterceptor = (...) => {
  const interceptionState = createInterceptionState();  // ← 상태가 여기서 생성됨!
  const pendingNavigation = createPendingNavigation();
  // 두 closure 상태가 URL 복원, handler 실행, replay 동안 유지되어야 함
  ...
  return (historyState) => {
    ...
    (async () => {
      ...
      if (shouldAllowNavigation) {
        interceptionState.confirmNextNavigation();  // ← 여기서 true 설정
...
[T0] 유저가 뒤로가기 클릭
     │
     ▼
[T1] interceptPopstate_A 호출 (state_A confirmation = false)
     │
     ▼
...
[T3] 유저가 "나가기" 클릭
     │
     ▼
[T4] state_A confirmation 설정
     │
     ▼
[T5] history.go(delta) 호출
     │
     ▼
[T6] 새 popstate 발생 → interceptPopstate_A 호출
     │
     ▼
[T7] state_A confirmation === true → 네비게이션 허용 ✅
```

**핵심:** 같은 `interceptPopstate_A` 인스턴스가 계속 사용되므로 `state_A` 상태가 유지됨.
...
[T2.2] Effect 재실행:
       ┌───────────────────────────────────────────────────────────┐
       │ cleanup: interceptPopstate_A 해제                           │
       │ setup:   interceptPopstate_B 새로 생성                      │
       │          (state_B: next-navigation confirmation = false)         │  ← 새 상태!
       └───────────────────────────────────────────────────────────┘
     │
     ▼
[T3] 유저가 "나가기" 클릭
     ...
     ▼
[T4] state_A confirmation 설정
     │  (하지만 interceptPopstate_A는 이미 해제됨!)
     │
     ▼
[T5] history.go(delta) 호출 → URL 변경됨
     │
     ▼
[T6] 새 popstate 발생 → interceptPopstate_B 호출 (현재 등록된 핸들러)
     │
     ▼
[T7] state_B.next-navigation confirmation === false → beforePopState가 false 반환
     │
     ▼
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  interceptPopstate_A (해제됨)     interceptPopstate_B (새로 생성)        │
│  ┌────────────────────────┐   ┌────────────────────────────────┐   │
│  │ state_A                │   │ state_B                        │   │
...
> 2. **새로운 `interceptPopstate` 클로저 생성** (`createPopstateInterceptor` 호출)
> 3. **새로운 `interceptionState` 생성** (모든 상태 초기화)
> 4. 이전 async 핸들러가 설정한 상태는 **이전 클로저에만 존재**
> 5. 새 popstate는 **새 클로저의 state**를 확인 → `next-navigation confirmation === false` → 차단
>
...
- `src/useInterceptPopState.helper/interception-state.ts` - `next-navigation confirmation` 상태 관리
