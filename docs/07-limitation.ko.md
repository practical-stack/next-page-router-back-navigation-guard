...
2. 핸들러에서 `/new-page`로의 네비게이션은 사용자 활성화 없이 발생하므로, 브라우저의 History Manipulation Intervention이 해당 엔트리들을 의심스러운 것으로 취급할 수 있습니다
3. 새 페이지에 핸들러가 없으면 추가적인 뒤로가기 네비게이션이 허용됩니다
4. 브라우저가 **예상치 못한 더 이전의 히스토리 항목**으로 이동합니다
...
이것이 발생하는 이유:
- 히스토리 스택에 `router.push()` 호출 **이전**의 항목들이 존재합니다
- `/new-page`는 추가적인 뒤로가기 네비게이션을 가로챌 핸들러가 없습니다
- 네비게이션이 예상 페이지 대신 더 이전의 히스토리 엔트리로 진행됩니다

### 근본 원인: 사용자 활성화 부재

핸들러 내에서 `router.push()`를 호출하는 것이 안전하지 않은 주된 이유는 사용자 활성화 없이 네비게이션이 발생하기 때문입니다. 핸들러는 `popstate` 콜백 내부에서 실행되며 — 사용자 제스처가 없습니다 — 따라서 브라우저의 History Manipulation Intervention이 결과 엔트리를 의심스러운 것으로 표시하여 브라우저 뒤로가기 버튼이 이를 건너뛰거나 잘못 처리할 수 있습니다.

부수적인 영향: 핸들러 내 `router.push()`로 추가된 페이지들은 push 이전의 히스토리 엔트리들 옆에 놓이게 되며, 핸들러가 없는 페이지는 해당 엔트리들을 통과하는 추가 네비게이션을 가로챌 수 없습니다. 이로 인해 예상치 못한 히스토리 위치에 도달할 수 있습니다.
...
| 사용자 활성화 | 사용자 활성화 없음 (핸들러 내부) | 사용자 버튼 클릭 (사용자 활성화 있음) |
...
**알려진 버그:**
- 브라우저 뒤로가기 버튼이 잘못된 페이지로 이동할 수 있음 (History Manipulation Intervention; 핸들러 컨텍스트에 사용자 활성화 없음)
- `router.back()` API는 동작하지만 브라우저 뒤로가기 버튼은 동작하지 않을 수 있음 (API는 해당 개입의 영향을 받지 않음)
- 사용자 활성화 없이 push된 엔트리는 브라우저가 의심스러운 것으로 취급하기 때문에 예상치 못한 페이지로 이동할 수 있음
