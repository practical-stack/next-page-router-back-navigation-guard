...
### 안전한 리다이렉트 패턴 (리다이렉트가 필요한 경우)

사용 사례에서 뒤로가기 시 리다이렉트가 필요하다면, 핸들러 내에서 직접 `router.push()`를 호출하는 대신 **모달 기반 패턴**을 사용하세요:

```typescript
// ✅ 안전함: 모달을 열고, 사용자가 네비게이션을 트리거
useRegisterBackNavigationHandler(() => {
  ...
  overlay.open(({ isOpen, close }) => (
    <RedirectModal
      isOpen={isOpen}
      close={close}
      onConfirm={() => router.push('/target-page')}  // 3. 사용자 클릭 → 네비게이션
    />
  ));
...
**왜 이 방식이 동작하는가:**
1. 핸들러가 `false`를 **동기적으로** 반환 → 뒤로가기 네비게이션 차단
2. 모달이 사용자에게 표시됨
3. 사용자가 버튼 클릭 → `router.push()`는 **새로운 사용자 발생 네비게이션**
4. 이것은 더 이상 핸들러 컨텍스트 내부가 아님
5. 브라우저가 이를 정당한 네비게이션으로 취급 (사용자 활성화 있음)

**안전하지 않은 패턴과의 주요 차이점:**

| 측면 | 안전하지 않은 패턴 | 안전한 패턴 |
|------|-------------------|------------|
| 핸들러 동작 | 핸들러 내에서 `router.push()` 호출 | 즉시 `false` 반환 |
| 네비게이션 트리거 | 핸들러 내부 (사용자 활성화 없음) | 사용자 버튼 클릭 (사용자 활성화 있음) |
| 새로고침 후 | ❌ 브라우저 뒤로가기 버튼 실패 가능 | ✅ 정상 동작 |
| 세션 토큰 | 이전/새 토큰 혼합으로 문제 발생 | 깔끔한 네비게이션 흐름 |

> 작동하는 구현은 [안전한 리다이렉트 패턴 예제](/redirect-safe)를 참조하세요.

### 안전하지 않은 리다이렉트 패턴 (권장하지 않음)

다음 패턴은 **지원되지 않으며** 페이지 새로고침 후 버그가 발생합니다:

```typescript
// ❌ 안전하지 않음: 핸들러 내에서 직접 router.push
useRegisterBackNavigationHandler(() => {
  router.push('/target-page'); // 이것은 문제를 일으킵니다!
  return false;
});
```

**알려진 버그:**
- 페이지 새로고침 후, 브라우저 뒤로가기 버튼이 잘못된 페이지로 이동할 수 있음
- `router.back()` API는 동작하지만 브라우저 뒤로가기 버튼은 동작하지 않을 수 있음
- 세션 토큰 불일치로 예상치 못한 페이지로 이동할 수 있음

> 이 버그를 재현하려면 [뒤로가기 시 리다이렉트 예제](/redirect)를 참조하세요.
...
## 3. 새로고침 후 앞으로가기가 뒤로가기로 처리됨

**페이지를 새로고침한 뒤에는 앞으로가기가 뒤로가기로 잘못 감지될 수 있습니다.**

### 왜 이런 일이 발생하는가

새로고침 후에는 Next.js Pages Router가 초기화 과정에서 `history.state`를 덮어쓰기 때문에, 이전 세션 토큰을 복원하는 것이 구조적으로 불가능합니다.

즉:

- `initializeHistoryStateSyncOnce()`는 항상 새로운 세션 토큰을 생성합니다
- 기존 히스토리 엔트리들은 여전히 이전 세션 토큰을 가지고 있습니다
- 새로고침 이후 발생하는 모든 `popstate`는 **세션 토큰 불일치** 상태가 됩니다
- 세션 토큰 불일치 상태에서는 **뒤로가기**와 **앞으로가기**를 신뢰성 있게 구분할 수 없습니다

그 결과, 라이브러리는 해당 네비게이션을 뒤로가기로 간주하고 현재 URL을 복원하기 위해 `history.go(1)`을 시도합니다.
...
사용자가 앞으로가기 클릭 (B → C)
    │
    ▼
popstate 발생, 브라우저 URL은 이미 C로 변경됨
    │
    ▼
세션 토큰 불일치 감지
    │
...
라이브러리가 "뒤로가기"로 간주
    │
    ▼
URL 복원을 위해 history.go(1) 실행
    │
    ├── C가 마지막 엔트리면: 아무 일도 일어나지 않음
    └── C 뒤에 D가 있으면: 브라우저가 의도치 않게 D로 이동할 수 있음
```
...
| 문제 | 설명 |
|------|------|
| 불필요한 핸들러 실행 | 앞으로가기 중에도 "페이지를 나가시겠습니까?" 같은 다이얼로그가 나타날 수 있음 |
| 사용자 혼란 | 사용자는 앞으로가기를 눌렀는데 UI는 뒤로가기처럼 동작함 |
| 잘못된 네비게이션 대상 | 이후 네비게이션이 허용되더라도 사용자가 의도한 페이지가 아닌 다른 페이지로 이동할 수 있음 |
| 히스토리 위치 드리프트 | 현재 대상이 히스토리 끝이 아니면 `history.go(1)`이 의도치 않은 이후 엔트리로 이동시킬 수 있음 |

### 왜 이것이 구조적 한계인가

이 문제는 라이브러리 내부에서 완전히 해결할 수 있는 단순 타이밍 버그가 아닙니다. 원인은 Pages Router 아키텍처에 있습니다:

- 브라우저는 JavaScript가 개입하기 전에 URL을 변경합니다
- Next.js Pages Router는 로드 시 `history.state`를 교체합니다
- 새로고침 후에는 이전 세션 토큰을 복원할 수 없습니다

따라서 **새로고침 이후 방향 감지는 근본적으로 신뢰할 수 없습니다**.

### 참고 자료

- [MDN: History.go()](https://developer.mozilla.org/en-US/docs/Web/API/History/go)
- [이 라이브러리가 필요한 이유 - 문제 3: 새로고침 및 외부 진입 감지](./01-why-this-library.ko.md#문제-3-새로고침-및-외부-진입-감지)
- [차단 시나리오 - 시나리오 4: Token Mismatch (새로고침 후 뒤로가기)](./02-blocking-scenarios.ko.md#시나리오-4-token-mismatch)

---

## 4. 삼성 인터넷 브라우저 "뒤로가기 리디렉션 차단" 기능
...
| 핸들러 내 router.push/replace | 예측 불가능한 네비게이션, 새로고침 후 브라우저 뒤로가기 버튼 실패 | **안전한 리다이렉트 패턴** 사용 (모달 + 버튼 클릭) |
| 새로고침 후 앞으로가기 | 세션 토큰 불일치로 뒤로가기처럼 처리될 수 있음 | 구조적 한계 (Pages Router에서 완전 해결 불가) |
...
4. **페이지 새로고침 후**, 라이브러리는 앞으로가기와 뒤로가기를 신뢰성 있게 구분할 수 없습니다
5. **iOS Safari**에는 popstate 이벤트에 대한 추가 제한이 있습니다

**가장 안전한 패턴**:
- 핸들러는 **확인 다이얼로그 표시** 또는 **오버레이 닫기**에만 사용하세요
- 리다이렉트가 필요하면 **안전한 리다이렉트 패턴** 사용: 모달을 열고, 사용자가 클릭하여 네비게이션
- 핸들러 내에서 직접 `router.push()`를 호출하지 **마세요**
