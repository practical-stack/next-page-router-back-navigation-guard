...
const isSessionBoundary =
  ...
  token !== current.sessionToken;  // 진짜 다른 세션
```
...
| `src/useInterceptPopState.helper/pending-navigation.ts` | URL 복원 중 대기하는 navigation 상태 |
| `src/useInterceptPopState.helper/rendered-history-entry-metadata-store.ts` | 렌더링된 history entry metadata 저장소 |
