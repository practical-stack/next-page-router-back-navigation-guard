...
1. 첫 번째 뒤로가기 → 핸들러 실행, 다이얼로그 표시, 차단 → 핸들러 삭제됨
2. 두 번째 뒤로가기 → 핸들러 없음, `preRegisteredHandler`가 다이얼로그 닫음, 차단
3. 세 번째 뒤로가기 → 핸들러 없음, 다이얼로그 없음 → 이전 페이지로 네비게이션

각 단계에서 URL 복원이 올바르게 처리되어 브라우저와 Next.js 상태가 동기화됩니다.
