...
2. The navigation from handler to `/new-page` happens without user activation, so the browser's History Manipulation Intervention may treat those entries as suspicious
3. Without a handler on the new page, further back navigation is allowed to proceed
4. The browser navigates to an **unexpected earlier history entry**
...
- The history stack contains entries from **before** the `router.push()` call
- `/new-page` has no handler to intercept further back navigation
- Navigation proceeds to an earlier history entry instead of the expected page

### Root Cause: Lack of User Activation

The primary reason calling `router.push()` inside a handler is unsafe is that the navigation happens without user activation. The handler fires inside a `popstate` callback — no user gesture is attached — so the browser's History Manipulation Intervention can flag the resulting entries as suspicious, causing the browser back button to skip or mishandle them.

A secondary effect: pages added via `router.push()` in a handler sit next to history entries from before the push, and pages without handlers cannot intercept further navigation through those entries. This can cause navigation to reach unexpected history positions.
...
| User activation | No user activation (inside handler) | User button click (user activation) |
...
**Known bugs:**
- Browser back button may navigate to wrong page (History Manipulation Intervention; no user activation in handler context)
- `router.back()` API may work while browser back button doesn't (API is not subject to the intervention)
- Navigation may jump to unexpected pages because entries pushed without user activation are treated as suspicious by the browser
